...
lblimg=Label(f, image=photo, bg='White')
lblimg.pack()
...
def ButtonClick():
    ...
    txtq4=txtquant4.get()
    txtq5=txtquant5.get()
    if not(txtq1.isdigit() or tkvar1.get()=="Item 1") or not(txtq2.isdigit() or tkvar2.get()=="Item 2") or not(txtq3.isdigit() or tkvar3.get()=="Item 3") or not(txtq4.isdigit() or tkvar4.get()=="Item 4") or not(txtq5.isdigit() or tkvar5.get()=="Item 5"):
        ...
        insertdb(i1,i2,i3,i4,i5,txtquant1.get(),txtquant2.get(),txtquant3.get(),txtquant4.get(),txtquant5.get())
...
def BillClick():
    #print("Nothing")
    sample=rand.get()
    if tkvar1.get()=='Item 1':
        tkvar1.set("")
    if tkvar2.get()=='Item 2':
        tkvar2.set("")
    if tkvar3.get()=='Item 3':
        tkvar3.set("")
    if tkvar4.get()=='Item 4':
        tkvar4.set("")
    if tkvar5.get()=='Item 5':
        tkvar5.set("")
    if total1.get()=='0.0':
        total1.set("")
    if total2.get()=='0.0':
    ...
    c.showPage()
    c.save()
    filepath="{}.pdf".format(sample)
    wb.open_new(r'{}.pdf'.format(sample))
...
def CalcTotal(r1,r2,r3,r4,r5):
    ...
    rate4.set(str(r4))
    rate5.set(str(r5))
    t1=float(int(q1.strip() or 0)*int(r1.strip() or 0))
    t2=float(int(q2.strip() or 0)*int(r2.strip() or 0))
    t3=float(int(q3.strip() or 0)*int(r3.strip() or 0))
    t4=float(int(q4.strip() or 0)*int(r4.strip() or 0))
    t5=float(int(q5.strip() or 0)*int(r5.strip() or 0))
    nt=(t1+t2+t3+t4+t5)
    perc=nt*5/100
    gt=(nt+perc)
    total1.set(str(t1))
...
values3={'Plain Dal','Dal Fry','Dal Tadka','Dal Maharani','Dal Makhani','Mix Veg','Veg Kadhai','Veg Handi','Veg Kofta','Chana Masala','Aalu Jeera','Dum Aalu','Aalu Gobi Masala','Paneer Butter Masala','Paneer Kofta','Paneer Bhurji','Palak Paneer','Mutter Paneer','Shahi Paneer','Sarso Ka Saag'}
values4={'Bundi Raita','Mix Veg. Raita','Onion Salad','Green Salad'}
values5={'Banana Milkshake','Chocolate Milkshake','Kiwi Milkshake','Strawberry Milkshake','Blackcurrant Milkshake','Mix Fruit Milkshake','Mango Milkshake','Blue Lagoon','Ginger & Mint','Fruit Punch','Lemon & Blackcurrant','Double Scoop(Choco,Vanilla)','Choco Blast Icecream','Cassata','Malai Kulfi(Pista)','Gulab Jamoon','Rasgulla'}
...
btn3=Button(f1,bd=5,fg="black",font=('arial',16,'bold'),text="Generate Bill",bg="#E5E7E9", justify='right',command=BillClick).grid(row=10,column=2)

btn4=Button(f1,bd=5,fg="black",font=('arial',16,'bold'),text="New Bill",bg="#E5E7E9", justify='right',command=Clear).grid(row=10,column=3)

#=========================================================================================Database====================================================================================================
def insertdb(A,B,C,D,E,qa,qb,qc,qd,qe):
    ...
    c.execute("insert into Menu values('%s','%s')" %('Aalu Gobi Masala','85'))
